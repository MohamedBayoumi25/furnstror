...
export class AllProductsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem(`productsInCart${localStorage.getItem("username")}`) == null || localStorage.getItem(`productsInCart${localStorage.getItem("usertoken")}`) == '') this.productsInCart = [];
    else {
  ...
  // ChangeProductsInCart(id: number) {
  //   !this.productsInCart.includes(id) ? this.productsInCart.push(id) : this.productsInCart = this.productsInCart.filter(p => p != id);
  //   this.tempCart[0].productsID = this.productsInCart;
  //   this.cartService.editcartProducts("1", this.tempCart[0]).subscribe({
  //     next: data => { }
  //   })
  // }
